...
type Summary = {
  byPhaseCategory: { phase: string; category: string; actual_jpy: string; count: string }[];
  byPhase: { phase: string; actual_jpy: string; count: string }[];
...
export default function Dashboard() {
  ...
  data.byPhase.forEach((r) => { phaseActuals[r.phase] = Number(r.actual_jpy); });

  // Budget by phase+category (Kenji only)
  const budgetMap: Record<string, number> = {};
  data.budget.filter(b => b.payer === "ケンジ").forEach((b) => {
    const key = `${b.phase}:${b.category}`;
    budgetMap[key] = (budgetMap[key] || 0) + Number(b.planned_jpy);
  });

  // Actual by phase+category
  const actualMap: Record<string, number> = {};
  data.byPhaseCategory.forEach((r) => {
    actualMap[`${r.phase}:${r.category}`] = Number(r.actual_jpy);
  });
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-md p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
          <StatCard label="予算" value={yen(BUDGET.total_available)} sub="使える額" color="text-ocean" />
          <StatCard label="実績合計" value={yen(kenjiActual)} sub={`${pctUsed.toFixed(1)}% 消化`} color="text-red-500" />
          <StatCard label="残高" value={yen(remaining)} sub={remaining >= 0 ? "黒字" : "赤字"} color={remaining >= 0 ? "text-green-600" : "text-red-600"} />
          <StatCard label="計画値" value={yen(BUDGET.kenji_total)} sub="現プラン予定額" color="text-gray-500" />
        </div>
        {/* 全体プログレスバー */}
          ...
          return (
            <div key={ph.id} className="bg-white rounded-2xl shadow-md p-5">
              ...
              <div className="flex justify-between text-sm mb-1">
                <span className="text-gray-600">実績 <strong className="text-red-500">{yen(actual)}</strong></span>
                <span className="text-gray-400">計画 {yen(planned)}</span>
              </div>
              <ProgressBar actual={actual} planned={planned} color={ph.color} />
              {actual > planned && (
                <p className="text-xs text-red-500 mt-1">⚠️ 予算超過 {yen(actual - planned)}</p>
              )}
            </div>
          );
        })}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400">
                        {e.expense_date?.split("T")[0]} · Phase {e.phase}
